...
class Handler(BaseHandler):
    ...
    @every(minutes=60)
    def on_start(self):
        self.crawl('://', callback=self.item_index, age=60, auto_recrawl=True, force_update=True)


    # 动态抓商品，从来没抓过的
    def item_index(self, response): 
        items = db_tmall.select('select id, itemId, type, noItem from `tmall_items` where crawled_at=0 order by type desc limit 0,100')
        for item in items:
            url_item = 'https://detail.tmall.com/item.htm?id=' + item['itemId']
            self.crawl(url_item, callback=self.item_page, age=600, force_update=True, max_redirects=10, save=item)


    # 抓天猫商品抽取数据，如果商品不存在 noItem + 1
    ...
    def item_page(self, response):
        ...
        if json_text:
            ...
            if json_data and 'itemDO' in json_data:
                auctionStatus = json_data['itemDO']['auctionStatus']
                itemNum = json_data['itemDO']['quantity']
                ...
                start_time = int(int(query['date'][0]) / 1000) if 'date' in query else 0
                startTime = datetime.utcfromtimestamp(start_time) + timedelta(hours = 8) if start_time else 0

                url_api = 'https:' + json_data['initApi']
                url_p = urlparse.urlparse(url_api)
                query = urlparse.parse_qs(url_p.query)
                kill_time = int(int(query['startSellTime'][0]) / 1000) if 'startSellTime' in query else 0
                secKillTime = datetime.utcfromtimestamp(kill_time) + timedelta(hours = 8) if kill_time else 0

...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    if db_engine is not None:
        raise DBError('db_engine is already initialized.')
    ...
    db_engine = _Engine(lambda: mysql.connector.connect(**params))
    # test connection...
    logging.info('Init mysql db_engine <%s> ok.' % hex(id(db_engine)))
...
def update_where(table, data, **kw):
    '''
    Execute update where SQL.
    data: a dict to update
    '''
    data_cols, data_args = zip(*data.iteritems())
    where_cols, where_args = zip(*kw.iteritems())
    sql = 'update `%s` set %s where %s' % (table, ' = ?, '.join(data_cols) + ' = ?', ' = ?, '.join(where_cols) + ' = ?')
    return _update(sql, *(data_args + where_args))
